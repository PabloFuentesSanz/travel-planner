...
import type { Trip, TripCollaborator } from '../types/database';
...
export class TripService {
  static async createTrip(data: CreateTripData): Promise<{ trip: Trip; error?: string }> {
    try {
      // Get current user
      ...
      if (data.collaboratorEmails && data.collaboratorEmails.length > 0) {
        const collaboratorPromises = data.collaboratorEmails.map(async (email) => {
  ...
  static async getTripById(tripId: string): Promise<{ trip: Trip | null; error?: string }> {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        throw new Error('Usuario no autenticado');
      ...
      const hasAccess = trip.user_id === user.id || 
        trip.trip_collaborators?.some((collab: any) => collab.user_id === user.id);
  ...
  private static async inviteCollaboratorByEmail(tripId: string, email: string) {
    try {
